...
import data from "@/content/niveles/preparatoria.json";
...
const ACC = {
  ...
  btnCls:  "bg-brand-red text-white hover:bg-[#c01020] shadow-md shadow-brand-red/20",
};
...
export function PreparatoriaPage() {
  ...
  return (
    <main>
      ...
      {/* ══════════════════════════════════════════════════════════════
          HERO
      ══════════════════════════════════════════════════════════════ */}
      <section className="relative bg-warm-white overflow-hidden" aria-label="Preparatoria">
        <div className="absolute -top-40 -right-40 w-[640px] h-[640px] rounded-full bg-brand-red/[0.05] pointer-events-none" aria-hidden="true" />
        <div className="absolute bottom-0 left-0 w-80 h-80 translate-y-1/3 -translate-x-1/3 rounded-full bg-brand-red/[0.04] pointer-events-none" aria-hidden="true" />
            ...
            <div className="lg:col-span-6 lg:-mr-8 xl:-mr-14">
              <div className="relative h-72 sm:h-96 lg:h-[580px] rounded-3xl overflow-hidden shadow-2xl shadow-ink/10">
                {data.heroImage ? (
                  ...
                  <div className="absolute inset-0 bg-gradient-to-br from-[#c01020] via-brand-red/80 to-brand-red/40" />
                )}
      ...
      {/* ══════════════════════════════════════════════════════════════
          NOS DISTINGUE — content left, image right
      ══════════════════════════════════════════════════════════════ */}
      <SectionContainer id="distingue" className="bg-white">
        <div className="grid lg:grid-cols-2 gap-14 lg:gap-24 items-center">

          <div>
            <p className="font-sans text-xs uppercase tracking-widest text-brand-red font-semibold mb-3">
              {data.displayName}
            </p>
          ...
          <div className="relative h-80 sm:h-[500px] rounded-3xl overflow-hidden">
            {data.distingueImage ? (
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-[#c01020] to-brand-red/40" />
            )}
          </div>
      ...
      {/* ══════════════════════════════════════════════════════════════
          ÁREAS DE DESARROLLO — Especialidad Traducción
      ══════════════════════════════════════════════════════════════ */}
      <SectionContainer id="areas" className="bg-warm-white">
        <div className="grid lg:grid-cols-[1fr_1.7fr] gap-14 lg:gap-20 items-start">
          ...
          <div className="lg:sticky lg:top-28">
            ...
            <h2 className="font-serif text-3xl sm:text-4xl font-bold text-ink leading-tight mb-4">
              {data.areasTitle}
            </h2>
            {data.areasSubtitle && (
      ...
      {/* ══════════════════════════════════════════════════════════════
          VALORES — 8 cards
          border-t-2 usado UNA SOLA VEZ en esta página
      ══════════════════════════════════════════════════════════════ */}
      <SectionContainer id="valores" className="bg-white">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-0">
          {data.valoresCards.map((card, i) => (
            <div key={i} className="border-t-2 border-brand-red pt-6 pb-8">
              <div className={cn("h-10 w-10 rounded-xl flex items-center justify-center mb-4", ACC.light)}>
                <AreaIcon emoji={card.icon} className={ACC.iconCls} />
              </div>
              <h3 className="font-serif text-base font-semibold text-ink mb-2 leading-snug">{card.title}</h3>
              <p className="font-sans text-sm text-muted leading-relaxed">{card.body}</p>
            </div>
          ))}
        </div>
        {data.valoresFooter && (
          <p className="font-sans text-sm text-muted italic text-center mt-10 max-w-2xl mx-auto">
            {data.valoresFooter}
          </p>
        )}
      </SectionContainer>
      ...
      {/* ══════════════════════════════════════════════════════════════
          ESPECIALIDAD ADMINISTRACIÓN — etapaItems como progresión semestral
      ══════════════════════════════════════════════════════════════ */}
      <SectionContainer id="administracion" className="bg-warm-white">
        <div className="grid lg:grid-cols-[1fr_1.6fr] gap-14 lg:gap-20 items-start">
          ...
          <div className="lg:sticky lg:top-28">
            ...
            <h2 className="font-serif text-2xl sm:text-3xl font-bold text-ink leading-tight mb-4">
              {data.etapaTitle}
            </h2>
            {data.etapaSubtitle && (
              <p className="font-sans text-sm text-muted leading-relaxed">
          ...
          <div className="space-y-8">
            {data.etapaItems.map((item, i) => (
              <div key={i} className="flex gap-6">
                <div className={cn("flex-shrink-0 h-12 w-12 rounded-2xl flex items-center justify-center text-lg font-bold", ACC.light)}>
                  <AreaIcon emoji={item.icon} className={ACC.iconCls} />
                </div>
                <div className="pt-1">
                  <p className={cn("font-sans text-xs font-bold uppercase tracking-widest mb-1", ACC.text)}>{item.title}</p>
                  <p className="font-sans text-sm text-muted leading-relaxed">{item.body}</p>
                </div>
              </div>
            ))}
      ...
      {/* ══════════════════════════════════════════════════════════════
          CLUB INTERACT
      ══════════════════════════════════════════════════════════════ */}
      <SectionContainer id="interact" className="bg-white">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {data.interactItems.map((item, i) => (
              <div key={i} className="flex items-start gap-4">
                <div className="h-10 w-10 rounded-xl bg-deep-green/10 flex items-center justify-center shrink-0">
                  <AreaIcon emoji={item.icon} className="text-deep-green" />
                </div>
                <div>
                  <h3 className="font-serif text-sm font-semibold text-ink mb-1 leading-snug">{item.title}</h3>
      ...
      {/* ══════════════════════════════════════════════════════════════
          DESARROLLO ACADÉMICO — Administración — image left, content right
      ══════════════════════════════════════════════════════════════ */}
      <SectionContainer id="academico" className="bg-warm-white">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">

          <div className="relative h-80 sm:h-[480px] rounded-3xl overflow-hidden order-2 lg:order-1">
            {data.trabajanImage ? (
          ...
          <div>
            ...
            <div className="space-y-4">
              {data.horarios.map((h, i) => (
                <div key={i} className={cn("flex items-start gap-4 rounded-xl p-4 border", ACC.light, "border-brand-red/20")}>
      ...
      {/* ══════════════════════════════════════════════════════════════
          CTA FINAL — Sin NivelNextLevel (último nivel)
      ══════════════════════════════════════════════════════════════ */}
      <section id="contacto-nivel" aria-label="Contáctanos" className="relative bg-deep-blue overflow-hidden">
        <div className="absolute inset-0 pointer-events-none select-none" aria-hidden="true">
          <div className="absolute -top-32 -right-32 w-[500px] h-[500px] rounded-full bg-brand-red/10" />
          <div className="absolute -bottom-40 -left-20 w-[400px] h-[400px] rounded-full bg-brand-red/5" />
        </div>
        <div className="relative z-10 px-4 sm:px-6 lg:px-8 py-16 sm:py-24 mx-auto max-w-xl text-center">
          <div className="h-0.5 w-8 bg-brand-red rounded-full mx-auto mb-8" aria-hidden="true" />
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a
              href="/#contacto"
              className={cn(
                "inline-flex items-center justify-center gap-2 font-semibold text-sm",
                "h-12 px-7 rounded-xl bg-brand-red text-white shadow-lg shadow-brand-red/20",
                "hover:bg-[#c01020] hover:shadow-xl active:scale-[0.97]",
      ...
      {/* NivelNextLevel devuelve null para Preparatoria (último nivel) */}
